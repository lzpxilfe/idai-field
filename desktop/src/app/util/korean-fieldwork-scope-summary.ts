    ...
    if (operationCount === 0) {
        return {
            ...
            detail: legacyRootRecordCount > 0
                ...
                : `${modeLabel} · 지도에서 GPS 임시 경계, SHP/DXF/GeoJSON/CSV, 지도 선택 기준 중 하나로 조사 경계를 먼저 만드세요.`,
            modeLabel,
            boundaryLabel,
            structureCount,
            evidenceCount,
    ...
    if (boundaryCount === 0) {
        ...
        return {
            ...
            detail: hasBoundarySummary
                ? `${modeLabel} · ${boundarySummary} 기준만 있음. GPS 임시 경계, SHP/DXF/GeoJSON/CSV, 지도 선택 기준 중 하나로 확정하세요.`
                : `${modeLabel} · GPS 임시 경계, SHP/DXF/GeoJSON/CSV, 지도 선택 기준으로 확정한 경계가 없습니다.`,
            modeLabel,
            boundaryLabel,
            structureCount,
            evidenceCount,
