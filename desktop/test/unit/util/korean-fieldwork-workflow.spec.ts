...
describe('korean-fieldwork-workflow', () => {
    ...
    it('guides the user from project creation to survey boundary setup', () => {
        ...
        expect(steps[2].detail).toBe('지도에서 조사 경계를 만들거나 SHP/DXF/GeoJSON/CSV·지도 선택 기준으로 구역을 기록하세요.');
        ...
        expect(steps[2].secondaryActionDetail).toBe(
            'SHP/DXF/GeoJSON/CSV는 데스크톱 가져오기에서 불러온 뒤 동기화하면 태블릿 지도에서도 조사 경계로 보입니다.'
        );
    ...
    it('keeps a boundary-summary-only project on the survey boundary confirmation step', () => {
        ...
        expect(steps[2]).toEqual(expect.objectContaining({
            label: '조사 구역',
            detail: 'A구역 북쪽 능선 기준만 있음. 지도에서 조사 경계를 만들거나 SHP/DXF/GeoJSON/CSV·지도 선택 기준으로 확정하세요.',
            action: { type: 'openMap' },
            actionLabel: '지도',
    ...
    it('asks to confirm the survey boundary once an operation exists with only a boundary summary', () => {
        ...
        expect(steps[2]).toEqual(expect.objectContaining({
            label: '조사 구역',
            detail: 'A구역 북쪽 능선 기준만 있음. 지도에서 GPS 임시 경계를 만들거나 SHP/DXF/GeoJSON/CSV·지도 선택 기준으로 확정하세요.',
            action: { type: 'openMap' },
            actionLabel: '지도',
            secondaryAction: { type: 'openImport' },
            secondaryActionDetail: 'SHP/DXF/GeoJSON/CSV는 데스크톱 가져오기에서 불러온 뒤 동기화하면 태블릿 지도에서도 조사 경계로 보입니다.',
