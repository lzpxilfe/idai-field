...
describe('makeKoreanFieldworkScopeSummary', () => {
    ...
    it('asks for an operation when mode exists without a fieldwork unit', () => {
        ...
        expect(summary).toMatchObject({
            tone: 'info',
            title: '조사 경계 필요',
            detail: '표본·시굴조사 · 지도에서 GPS 임시 경계, SHP/DXF/GeoJSON/CSV, 지도 선택 기준 중 하나로 조사 경계를 먼저 만드세요.',
            modeLabel: '표본·시굴조사',
            boundaryLabel: '경계 없음',
    ...
    it('asks for a survey boundary once the operation exists without boundary data', () => {
        ...
        expect(summary).toMatchObject({
            tone: 'warning',
            title: '조사 구역 필요',
            detail: '표본·시굴조사 · GPS 임시 경계, SHP/DXF/GeoJSON/CSV, 지도 선택 기준으로 확정한 경계가 없습니다.',
            modeLabel: '표본·시굴조사',
            boundaryLabel: '경계 없음',
            actionLabel: '지도',
            action: { type: 'openMap' },
    ...
    it('asks to confirm the map boundary when only the project boundary summary exists', () => {
        ...
        expect(summary).toMatchObject({
            tone: 'warning',
            title: '조사 구역 확정 필요',
            detail: '표본·시굴조사 · A구역 북쪽 능선 기준만 있음. GPS 임시 경계, SHP/DXF/GeoJSON/CSV, 지도 선택 기준 중 하나로 확정하세요.',
            modeLabel: '표본·시굴조사',
